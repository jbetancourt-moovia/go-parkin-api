...
func (r *CustomerRepository) GetAll(ctx context.Context) (*[]models.Customer, error) {
	rows, err := r.db.Query(ctx, `SELECT id, first_name, last_name, phone, email FROM customers ORDER BY id`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&c.ID, &c.FirstName, &c.LastName, &c.Phone, &c.Email); err != nil {
			return nil, err
		}
...
func (r *CustomerRepository) Create(ctx context.Context, c *models.CustomerCreate) error {
	query := `INSERT INTO customers (first_name, last_name, phone, email)
	          VALUES ($1, $2, $3, $4)`
	_, err := r.db.Exec(ctx, query, c.FirstName, c.LastName, c.Phone, c.Email)
	return err
}

func (r *CustomerRepository) GetByID(ctx context.Context, id int) (*models.Customer, error) {
	var c models.Customer
	err := r.db.QueryRow(ctx, `SELECT id, first_name, last_name, phone, email FROM customers WHERE id = $1`, id).
		Scan(&c.ID, &c.FirstName, &c.LastName, &c.Phone, &c.Email)
	if err != nil {
		return nil, err
	}
	return &c, nil
